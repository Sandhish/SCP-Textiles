...
const FeaturedProducts = ({ onAddToWishlist }) => {
    ...
    useEffect(() => {
        const fetchFeaturedProducts = async () => {
            try {
                ...
                const featuredProducts = response.data.filter(product => product.tag === "featured");
    ...
    if (loading) return <div className={styles.loading}>Loading featured products...</div>;
    if (error) return <div className={styles.error}>{error}</div>;
    if (products.length === 0) return <div className={styles.noProducts}>No featured products found.</div>;
                            ...
                            {products.map((product) => (
                                <Link to={`/product/${product._id}`} key={product._id} className={styles.product} >
                                    <div className={styles.imageContainer}>
                                        ...
                                        >
                                            {wishlist.has(product._id) ? <FaHeart className={styles.wishlistIconActive} /> : <FaRegHeart className={styles.wishlistIcon} />}
                                        </button>
                                        ...
                                        <div className={styles.rating}>
                                            {[...Array(5)].map((_, i) => (
                                                <svg key={i} xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24"
                                                    className={styles.stars} fill={i < Math.floor(product.rating || 0) ? "currentColor" : "none"}
                                                    stroke="currentColor" strokeWidth="2">
                                                    <polygon points="12 2 15.09 8.26 22 9.27 17 14.14 18.18 21.02 12 17.77 5.82 21.02 7 14.14 2 9.27 8.91 8.26 12 2"></polygon>
                                                </svg>
                                            ))}
                                            <span className={styles.reviews}>({product.reviews?.length || 0})</span>
                                        </div>
